...
5. **補完ログのファイル保管（必須）**
- AIによる補完が1つでも発生した場合、必ず `Specs/Assumptions/` 配下に補完ログを保存する。
- ファイル名は `YYYYMMDD_HHMM_<topic>_assumptions.md` とする。
- 記載項目は次を必須とする: 対象spec、補完内容、理由、影響範囲（どのファイル/挙動に影響するか）。
- 補完ログ本文は、小学校高学年が読めるやさしい日本語で書く。
- 難しい言葉を使う場合は、かっこで短く言いかえを付ける（例: 影響範囲（どこが変わるか））。
- 1文はできるだけ短くし、専門用語や英語の連続使用を避ける。
...
8. **仕様外機能が必要な場合の助言（必須）**
- 仕様どおりでは破綻する、またはバグ回避できないと判断した場合は、勝手に仕様外機能を実装しない。
- 次の形式で、最小限の追加案を必ず提示する。
	- 【仕様外の必要性】: なぜ必要か
	- 【最小追加案】: 追加する機能を1つに絞って説明
	- 【追加しない場合のリスク】: 何が起きるか（バグ、進行不能、学習体験の低下など）
	- 【影響範囲】: どこが変わるか（ファイル・挙動）
- 追加案の説明は、小学校高学年が読めるやさしい日本語で書く。
- この助言を出した場合は、必ず `Specs/Assumptions/` 配下に助言ログを保存する。
- ファイル名は `YYYYMMDD_HHMM_<topic>_change-proposal.md` とする。
