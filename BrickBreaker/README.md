...
- js/game.js: ゲーム全体のまとめ役（状態管理とメインループ）
- js/config.js: 調整可能な設定値
- js/collision.js: 当たり判定の計算（汎用・状態を持たない）
- js/entities.js: パドル・ボール・ブロックの初期生成
- js/render.js: Canvas への描画
- js/input.js: キーボード・タッチ入力の状態管理
- js/events.js: ゲーム内イベントの発行・購読の仕組み

## ステージ管理
- ステージごとのブロック配置は `js/layouts.js` に静的データとしてまとめています。
- 配置の編集は `scripts/layout_masks/` を更新し、`python3 BrickBreaker/scripts/generate_layouts.py` で再生成する運用です。
...
### ヒント1: js/config.jsだけで進める改造（やりやすい順）
...
6. ステージの並びを変える
- 変更場所: `scripts/layout_masks/*.txt`（編集後に `python3 BrickBreaker/scripts/generate_layouts.py` を実行）
- ヒント: `.` は空きマス。`A` や `B` を増減して、自由に配置を作れます

7. ステージを追加する
- 変更場所: `scripts/layout_masks/` に新しいマスク追加、`scripts/generate_layouts.py` の `STAGE_ORDER` 追加、`js/config.js` の `stages` 追加
...
- 入口: `js/config.js` に `itemDropRate` や `itemTypes` を追加し、`js/entities.js` で生成、`js/game.js` で効果を反映

2. ボール数を増やす（マルチボール）
- 例: 特定条件でボールを2個/3個に増やす（例: ステージ開始10秒後、特定ブロック破壊時）
- 入口: `js/config.js` に `maxBalls` や `multiBallTrigger` を追加し、`js/entities.js` でボールを作り、`js/game.js` で同時に更新

3. 時間イベントを追加する
- 例: 10秒だけパドルが広くなる、20秒ごとにボール速度が少し上がる
- 入口: `js/config.js` に `eventIntervalSec` / `eventDurationSec` を追加し、`js/game.js` の更新ループで扱う

4. ブロック効果イベントを追加する
- 例: 特定タイプのブロックを壊すと「全ブロックの色が変わる」「次の5秒だけ得点2倍」
- 入口: `blockTypes` に `onBreakEffect` を追加し、`js/game.js` 側で効果を切り替える
