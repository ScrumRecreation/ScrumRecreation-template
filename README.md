...
このリポジトリの主要ディレクトリ・ファイルは以下のとおりです。

| パス | 概要 |
| :--- | :--- |
| `README.md` | プロジェクト全体の説明（本ファイル） |
| `README_kids.md` | 子ども向けのやさしい説明 |
| `Schedule.md` | ワークショップ進行のスケジュール |
| `BrickBreaker/` | ブロック崩しゲーム本体 |
| `BrickBreaker/index.html` | ゲーム起動用エントリーポイント |
| `BrickBreaker/css/` | スタイル定義 |
| `BrickBreaker/js/` | ゲームロジック（描画・入力・当たり判定など） |
| `BrickBreaker/scripts/` | ステージレイアウト生成用スクリプトと素材 |
| `Flyers/flyer.html` | チラシ用HTML |
| `scripts/` | ドキュメント整形などの補助スクリプト |
| `Specs/` | 仕様サンプル・前提条件・補足資料 |
| `Templates/` | バックログ/レビュー/レトロ/仕様のテンプレート |
